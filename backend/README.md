...
| 缺口 | 现状 | 建议 |
|---|---|---|
| 登录失败场景未单独定义错误码 | 当前统一返回 `401 + 通用文案` | 接口文档可补 `LOGIN_FAILED(4011)` / `ACCOUNT_LOCKED(4012)` |
| 限流触发 429 的阈值与重试策略 | Nginx `limit_req 10r/s` 触发即 429, 后端未参与限流 | 接口文档应明确 `Retry-After` 头, 测试组补 TC-007(429 场景) |
...
| 报告生成时间跨度上限 | 后端硬编码 30 天, 与需求 §4.5 一致 | 测试组应补 TC-009(30 天临界) |
| 离线告警与 STATUS 告警合并策略 | 每条 STATUS 都生成新告警, 未做"相同设备同类型未处置去重" | 接口文档 §2.2 应明确"同一类型 NEW 告警去重窗口" |
